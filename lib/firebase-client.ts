...
function emulatorAddress(name: string, fallback: string) {
...
function connectLocalEmulators(app: ReturnType<typeof getApp>) {
  ...
  const authAddress = emulatorAddress("NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_HOST", "127.0.0.1:9099");
  const functionsAddress = emulatorAddress("NEXT_PUBLIC_FIREBASE_FUNCTIONS_EMULATOR_HOST", "127.0.0.1:5001");
